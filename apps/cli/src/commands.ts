 ...
 *
 * A slash command *is* a route path (see `routes.ts`) — typing it and submitting
 * navigates there. This list is the discoverable subset shown in the autocomplete
 * menu when the prompt begins with `/`; each entry carries a one-line description.
 * Keep it in sync with the routes worth surfacing to the user.
 */
import { ROUTES } from "./routes";

export type SlashCommand = {
  /** The full command as typed, e.g. `/model`. Also the route path it navigates to. */
  command: string;
